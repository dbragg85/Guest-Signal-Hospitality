...
import { getPublishedNewsletters, getInsightPath } from "@/lib/newsletter/content";
import { Section } from "@/components/Section";

export const metadata = {
  title: "Hospitality Signals Newsletter Archive",
  description:
    "Weekly hospitality operational intelligence for restaurant operators. Browse the archive or read the latest briefs on guest experience, service recovery, and reputation systems.",
  ...
  return (
    <div className="bg-gradient-to-b from-stone-100 via-white to-stone-50">
      <section className="border-b border-stone-200/80 bg-[radial-gradient(circle_at_top_right,_rgba(245,158,11,0.16),_transparent_40%),radial-gradient(circle_at_top_left,_rgba(2,132,199,0.14),_transparent_35%)]">
        <NewsletterArchiveHero />
      </section>

      <Section title="Published issues" kicker="Archive">
        <div className="mx-auto max-w-4xl space-y-4">
          <p className="text-center text-sm text-slate-600">
            Articles now live at short URLs under{" "}
            <Link href="/insights/" className="font-semibold text-amber-900 underline">
              /insights/
            </Link>
            . Legacy newsletter links redirect automatically.
          </p>
          {items.length === 0 ? (
              ...
              >
                {item.frontmatter.heroImage ? (
                  <img
                    ...
                    alt={`${item.frontmatter.title} — hospitality intelligence brief`}
                ...
                <div className="mt-5">
                  <Link href={getInsightPath(item.frontmatter.slug)} className="btn-primary inline-block">
...
function NewsletterArchiveHero() {
  return (
    <div className="mx-auto max-w-3xl px-4 py-14 text-center sm:px-5">
      ...
      <p className="mt-4">
        <Link href="/insights/" className="text-sm font-semibold text-amber-900 underline underline-offset-2">
          Go to the Hospitality Signals hub
        </Link>
      </p>
    </div>
  );
