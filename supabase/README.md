...
3. **Submit a test using a plan link** — not the short contact page. Example: `https://your-domain.com/snapshot/` for the free snapshot (or a paid plan key from the Plans page at `/services/inquiry/?plan=signal_monitor`, etc.). Plain `/contact` saves `inquiry_plan=general` and the snapshot **GitHub Action will skip it**.
